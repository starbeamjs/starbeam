...
test("a simple element containing a text node (dynamic) ", ({ dom, test }) => {
    ...
    let element = test.buildElement("div", { children: [dom.text(name)] }, Expects.dynamic);
...
test("a simple element containing a text node (static) ", ({ dom, test }) => {
    ...
    let element = test.buildElement("div", {
        attributes: { title },
        children: [dom.text(name)],
    }, Expects.constant);
    test.render(element, Expects.constant.html(`<div title="${TITLE}">${NAME}</div>`));
});
test("a simple element with an attribute (dynamic) ", ({ dom, test }) => {
    ...
    let result = test.render(element, Expects.dynamic.html(`<div title="${TITLE}">${NAME}</div>`));
    result.update([name, SHORT_NAME], Expects.html(`<div title="${TITLE}">${SHORT_NAME}</div>`));
});
test("(smoke test) a dynamic element with a few children and a few attributes", ({ dom, test, }) => {
    ...
    let result = test.render(element, Expects.dynamic.html(`<div title="${TITLE}" class="person" style="${STYLE}">${FIRST_NAME} ${LAST_NAME} <span>(name)</span> -- Over and Out</div>`));
    result.update([firstName, SHORT_NAME], Expects.html(`<div title="${TITLE}" class="person" style="${STYLE}">${SHORT_NAME} ${LAST_NAME} <span>(name)</span> -- Over and Out</div>`));
});
