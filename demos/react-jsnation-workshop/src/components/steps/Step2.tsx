  ...
  return useReactive(() => {
    const profile =
      user.current === undefined ? (
        <pre>Invalid user id {user.index + 1}</pre>
      ) : (
    ...
    return (
      <>
        <Field
          type="number"
          label="User ID"
          onUpdate={(id) => void user.choose(Number(id) - 1)}
          value={user.index + 1}
        />
        {profile}
      </>
...
function Profile({ user }: { user: ApiUser }): JSX.Element {
  return (
    <pre>
      <Jsonify value={user} />
    </pre>
...
const CurrentUser = Resource(() => {
  const currentIndex = Cell(0);

  return {
    get current() {
      return users[currentIndex.current];
    },
