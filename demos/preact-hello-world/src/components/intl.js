...
export function formatLocale(localeName) {
...
function localeInfo(locale, code, type) {
    if (code) {
        return new Intl.DisplayNames([locale.language], { type }).of(code);
    }
}
export function timeZoneName(locale, timeZone) {
    ...
    if (tz === undefined) {
        throw Error(`Could not find time zone name for specified time zone: ${timeZone}`);
    }
    return tz.value;
}
