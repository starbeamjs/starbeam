import { TrackedMap } from "./builtins/map.js";
import { TrackedSet } from "./builtins/set.js";
...
export class IndexMap<K, V> {
  static create<A, B>(): IndexMap<A, B> {
    return new IndexMap<A, B>(
      new TrackedMap<A, Set<B>>(),
      new TrackedMap<B, Set<A>>()
    );
  }
  ...
  add(a: K, b: V): void {
    const set = this.#byKey.get(a);
    if (set === undefined) {
      this.#byKey.set(a, new TrackedSet([b]));
    } else {
      set.add(b);
    }
    ...
    const set2 = this.#byValue.get(b);
    if (set2 === undefined) {
      this.#byValue.set(b, new TrackedSet([a]));
    } else {
      set2.add(a);
    }
  }
