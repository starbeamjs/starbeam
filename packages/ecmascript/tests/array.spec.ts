...
import ecma from "@starbeam/ecmascript";
...
describe("TrackedArray", () => {
  ...
  test("setting an item to the same value doesn't invalidate the index", () => {
    const array = ecma.array<string>(["hot dogs"]);
  ...
  test("truncating an array", () => {
    const array = ecma.array<string>(["hot dogs", "brie", "chevre"]);
