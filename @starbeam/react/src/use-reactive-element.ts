...
export function useReactiveElement<I extends Inputs>(
  ...
  const { current: resource } = useResource
    ...
    .update(() => {
       ...
       */
      LOGGER.debug.log(SHEET.green(`instance is updating: ${description}`));
    })
    .as(description)
    .notifier(() => setNotify({}))
    .on({
      attached: ({ element }) => {
        ...
        LOGGER.debug.log(
          ...
          SP,
          SHEET.green(`attaching instance`)
        );
      ...
      ready: ({ element }) => {
        ...
        LOGGER.debug.log(SHEET.green(`instance is ready: ${description}`));
      ...
      deactivate: ({ element }) => {
        LOGGER.debug.log(
          SHEET.inert(description),
          SP,
          SHEET.red(`deactivating instance`)
        );
  ...
  function createReactiveElement({
    ...
    LOGGER.debug.log(SHEET.green(`initializing instance: ${description}`));
